...
sub create_vms {
   ...
   foreach (@vms) {
      ...
      my $nic_poweron = 1;
      my $resourcepool = 'shared-servers';
      my $vmfolder = 'shared-servers';
      ...
      if ($_->findvalue('ResourcePool')) {
         $resourcepool = $_->findvalue('ResourcePool');
      }
      if ($_->findvalue('VMFolder')) {
         $vmfolder = $_->findvalue('VMFolder');
      }
...
sub create_vm {
   ...
   my $resourcepool_view = Vim::find_entity_view(view_type => 'ResourcePool',
                                filter => {'name' => $args{resourcepool}});

   if (!$resourcepool_view) {
       Util::trace(0, "\nError creating VM '$args{vmname}': "
                    . "Resource Pool '$args{resourcepool}' not found\n");
       return;
   }

   my $vmfolder_view = Vim::find_entity_view(view_type => 'Folder',
                                filter => {'name' => $args{vmfolder}});

   if (!$vmfolder_view) {
       Util::trace(0, "\nError creating VM '$args{vmname}': "
                    . "VM Folder '$args{vmfolder}' not found\n");
       return;
   }
   ...
   if($vm_folder_view->can("childEntity")){
     my $children = $vm_folder_view->childEntity;
	for my $child (@{$children}) {
          next if $child->type eq 'VirtualMachine';
          my $child_folder = Vim::get_view(mo_ref => $child);
          if($child_folder->name eq $args{'vmfolder'}){
            $vmfolder_view = $child_folder;
          }
        }
   }

   eval {
      $vmfolder_view->CreateVM(config => $vm_config_spec,
                               pool => $resourcepool_view);
